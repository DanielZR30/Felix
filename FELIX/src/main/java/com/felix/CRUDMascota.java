...
public class CRUDMascota {
    ...
    public String buscarNombre(Archivos objArch,String idDueno, String n) 
    { 
        String id = "";
        ...
        try {
            ...
            while (Reg[0] != null) //mientras not EOF()
            {
                ...
                //si la ID que estamos buscando es igual a la ID que extraimos del archivo en Reg
                if(idDueno.equalsIgnoreCase(dueno)&&nombre.equalsIgnoreCase(n))
                {id = IDM;}//fin si
                //se lee el otro registro para que termine secuencialmente la lectura del archivo texto
                Reg = objArch.LeerRegistro(5);
            }//fin mientras
        ...
        if(id.isBlank()){
            ...
            objM = IngresarMascota(objArch,idDueno);
            id = objM.getID();
        }
       return id;
   ...
   */
    public Mascota IngresarMascota(Archivos objArch,String ID) 
    {
       Mascota objm = new Mascota();
